...
export default class UserService extends Services {
  ...
  async login(user) {
    try {
      ...
      // if(userExist && passValid) return this.generateToken(userExist); // asi me sirve para el token en controller
      if(userExist && passValid){
        const token = this.generateToken(userExist)
        return {user:userExist, token}; //asi me sirve para validar el initialize
      }   
    } catch (error) {
      throw new Error(error);
    }
